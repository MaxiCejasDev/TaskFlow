...
export default function TaskEdit ({handleOpenTaskEditor, openTaskEditor,handleEditTask,taskId} : TaskEditor){
    const [newTaskText, setNewTaskText] = useState('')
    const handleUpdateTask = (taskId : number , taskTextEdit : string)=>{
        handleEditTask(taskId,taskTextEdit)
        setNewTaskText('')
    }
    return(
        <div className={openTaskEditor?"fixed bg-[rgba(0,0,0,.5)] h-screen w-full top-0 left-0 z-10 flex justify-center items-center":"hidden"}>
            <div className="bg-white h-1/4 w-1/4 flex justify-center items-center gap-x-2 rounded-[12px] relative">
                ...
                <input onChange={(e)=>setNewTaskText(e.target.value)} className="border-2 border-blue-light" placeholder="Ingresar cambio" type="text" />
                <button onClick={()=>{
                    handleUpdateTask(taskId, newTaskText)
